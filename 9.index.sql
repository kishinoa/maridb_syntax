...
-- 만약 where조건에서 2컬럼으로 조회시에, 1컬럼에만 index가 있다면 -> 인덱스 우선 탐색
select * from author where name = 'hong1' and email = 'hong1@naver.com'

-- 만약 where조건에서 2컬럼으로 조회시에, 2컬럼 각각에 index가 있다면 -> db 엔진에서 최적의 알고리즘 실행
select * from author where id = 1 and email = 'hong1@naver.com';
